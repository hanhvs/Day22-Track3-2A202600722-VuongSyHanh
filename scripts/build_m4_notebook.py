#!/usr/bin/env python3
"""Generate colab/Lab22_DPO_M4.ipynb — an Apple Silicon (M1/M2/M3/M4) compatible
variant of the Day-22 DPO lab.

Why a separate builder? The T4/BigGPU notebooks depend on Unsloth + bitsandbytes
4-bit kernels, which have no MPS (Metal) backend. This M4 variant rewrites every
stage to run on plain transformers + peft + trl over the `mps` device, full
precision (fp32) LoRA, no quantization. Run:

    python scripts/build_m4_notebook.py

and it writes/overwrites the notebook deterministically.
"""
...
OUT = REPO / "colab" / "Lab22_DPO_M4.ipynb"
...
cells.append(code(
    ...
    "SFT_DATASET = os.environ.get(\"SFT_DATASET\", \"argilla/ultrafeedback-binarized-preferences-cleaned\")\n"
